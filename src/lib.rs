...
pub trait IsRomanceVowel: private::Sealed {
    /// Return `true` if and only if `self` appears to be a "vowel" in a Romance language.
    ///
...
/// Return `true` if and only if `c` is in `vowels` after transformation via `decompose_starting
